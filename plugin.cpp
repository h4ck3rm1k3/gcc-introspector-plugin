...
class VisitInReport : public TypeFinished
{
void name(tree t)
  {
    tree d =DECL_NAME(t);
...
public:
  void record(tree t)
  {
    printf ("starting\n");
    name (t);
    generic_callback(t, (void*)"user_data");
...
static void generic_callbackPLUGIN_FINISH_TYPE (tree t, void *_)
{
  ...
  if (t)
    {
      TypeFinished block;
      block.check_record(t);
    }

  //gcc_report_tree_visit(t);
}

void print_data(const char *c_gcc_data)
    {
      int x;
...
*/
#define DEFEVENTSPECIAL(X)   register_callback (plugin_info->base_name,  X,  (plugin_callback_func)&generic_callback ##X, (void*) #X );
//#define DEFEVENT(X)   
